...
Sprint requires forward input, grounded standing movement, no aim, no roll and no cover. Sprint, roll and falling block combat. Melee locks travel; low-cover and active-peek melee are rejected, and crouched/pending-crouch melee is disabled because its acquired punches are standing full-body actions. Space first detaches if attached, otherwise tries cover in camera-yaw direction, then falls back to grounded jump. J requests jump without a cover search. E/controller left shoulder requests the separate dodge roll. There is no vault branch.
