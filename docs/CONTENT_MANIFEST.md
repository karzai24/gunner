...
## Xbox input composition

`Content/Gunner/Motion/Input/IMC_Xbox`, `DA_XboxInput` and `IA_CycleWeapon` add the complete controller layout while retaining keyboard bindings. `BP_WardenMotion` selects this input config; its mesh, animation and movement settings are unchanged by this input pass. Original `DA_MotionInput` / `IMC_Motion` assets are preserved, and the pre-input-update character backup is under ignored `Saved/AuthoringDrafts`. These are project-owned input assets with no new third-party dependencies.
