"""Run two rendered -GunnerTraversalSmoke sessions against transient native fixtures.

Require the native probe to complete; premature PIE teardown is a failure.
The editor restores its foreground-throttle setting before exiting.
"""
import time
import unreal as u
...
def tick(delta):
    ...
    if state['phase'] == 'start' and elapsed > 3:
        ...
        if state.get('saw_world') and not levels.is_in_play_in_editor():
            u.log_error('GUNNER_TRAVERSAL_PIE_ABORTED: play stopped before the probe completed')
            finish()
            return
        world = editor.get_game_world()
        if world:
            state['saw_world'] = True
            probes = u.GameplayStatics.get_all_actors_of_class(world, u.GunnerTraversalProbe)
            if len(probes) == 1 and not probes[0].is_actor_tick_enabled():
                failures = probes[0].get_failure_count()
                state['cycles'] += 1
                state['failures'] += failures
                u.log(f'GUNNER_TRAVERSAL_PIE_CYCLE_FINISHED {state["cycles"]} failures={failures}')
                levels.editor_request_end_play()
                state.update(phase='stopping', at=time.monotonic())
                return
        if elapsed > 180:
            state['failures'] += 1
            u.log_error('GUNNER_TRAVERSAL_PIE_TIMEOUT')
            levels.editor_request_end_play()
            state.update(phase='failed_stopping', at=time.monotonic())
    elif state['phase'] in ('stopping', 'failed_stopping'):
        if elapsed > 10:
            u.log_error('GUNNER_TRAVERSAL_PIE_STOP_TIMEOUT')
            finish()
        elif elapsed > 1 and not levels.is_in_play_in_editor():
            if state['phase'] == 'failed_stopping':
                finish()
            elif state['cycles'] == 2:
                message = f'GUNNER_TRAVERSAL_PIE_COMPLETE cycles=2 failures={state["failures"]}'
                if state['failures']:
                    u.log_error(message)
                else:
                    u.log(message)
