"""Run two rendered PIE sessions with -GunnerCrouchReloadSmoke, then close the test editor.

The native probe drives player input and observes evaluated poses, montage lifecycle,
and ammunition. Completion is not acceptance unless both cycles report zero failures.
"""
import time
import unreal as u
    ...
    if state['phase'] == 'start' and elapsed > 3:
        ...
        if world:
            ...
            if len(probes) == 1 and not probes[0].is_actor_tick_enabled():
                ...
                u.log(f'GUNNER_CROUCH_RELOAD_PIE_CYCLE_FINISHED {state["cycles"]} failures={failures}')
                levels.editor_request_end_play()
                state.update(phase='stopping', at=time.monotonic())
                return
        if elapsed > 150:
            state['failures'] += 1
            u.log_error('GUNNER_CROUCH_RELOAD_PIE_TIMEOUT')
            levels.editor_request_end_play()
            state.update(phase='failed_stopping', at=time.monotonic())
    elif state['phase'] in ('stopping', 'failed_stopping'):
        if elapsed > 10:
            u.log_error('GUNNER_CROUCH_RELOAD_PIE_STOP_TIMEOUT')
            finish()
        elif elapsed > 1 and not levels.is_in_play_in_editor():
            if state['phase'] == 'failed_stopping':
                finish()
            elif state['cycles'] == 2:
                message = f'GUNNER_CROUCH_RELOAD_PIE_COMPLETE cycles=2 failures={state["failures"]}'
                if state['failures']:
                    u.log_error(message)
                else:
                    u.log(message)
