"""Run two real PIE sessions of -GunnerLookSmoke, then close the test editor.

Synthetic Enhanced Input checks cover the gameplay path, not physical mouse hardware.
"""
import time
import unreal as u
...
u.log(f'GUNNER_LOOK_MAC_NEW_MOUSE_BACKEND value={backend}')
...
def tick(delta):
    ...
    if state['phase'] == 'start' and elapsed > 3:
        ...
        if world:
            probes = u.GameplayStatics.get_all_actors_of_class(world, u.GunnerLookProbe)
            if len(probes) == 1 and not probes[0].is_actor_tick_enabled():
                state['cycles'] += 1
                u.log(f'GUNNER_LOOK_PIE_CYCLE_FINISHED {state["cycles"]}')
                levels.editor_request_end_play()
                state.update(phase='stopping', at=time.monotonic())
                return
        if elapsed > 90:
            u.log_error('GUNNER_LOOK_PIE_TIMEOUT')
            levels.editor_request_end_play()
            finish()
    elif state['phase'] == 'stopping' and elapsed > 1 and not levels.is_in_play_in_editor():
        if state['cycles'] == 2:
            u.log('GUNNER_LOOK_PIE_COMPLETE cycles=2')
            finish()
        else:
